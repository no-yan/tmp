...
"#;
    ...
    let original_segments = MarkdownProcessor::parse(test_markdown)
    ...
    let original_order: Vec<String> = original_segments
        ...
        .map(|s| match s {
            ...
            Segment::Paragraph { text } => format!("Paragraph({}...)", &text[..20.min(text.len())]),
        ...
        .enumerate()
    {
        match (orig, trans) {
            ...
            (
                Segment::CodeBlock { language: l1, code: c1 },
                Segment::CodeBlock { language: l2, code: c2 },
            ) => {
...
"#;
    ...
    for (i, (s1, s2)) in first_segments.iter().zip(second_segments.iter()).enumerate() {
        assert_eq!(
            std::mem::discriminant(s1),
            std::mem::discriminant(s2),
            "Segment types should match at index {} (cache hit)", i
        );
    }
